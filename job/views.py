...
from .forms import CreateJobForm, JobDescriptionForm, SkillForm, UpdateJobForm
...
from users.email_tasks import send_application_email, send_new_job_email
...
def create_job(request):
    ...
    return render(
        request, "dashboard/recruiterDashboard/addJob.html", {"form": form, "company": company}
    )
...
def job_type_view(request):
    ...
    return render(
        request, "dashboard/recruiterDashboard/jobProps.html", {"form": form, "company": company}
    )
...
def enter_job_description(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            combined_text = (description or "") + (responsibilities or "") + (ideal_candidate or "")
            ...
            request.session[
                "selected_category"
            ] = job.category.id  # Assuming job has a category field
    ...
    return render(
        request,
        "dashboard/recruiterDashboard/jobDescription.html",
            ...
            try:
                ...
                if request.method == "POST":
                    ...
                    if form.is_valid():
                        ...
                        messages.info(
                            request,
                            "Your job has been successfully created, please be patient while we process the job assessment!",
                        )
                ...
                return render(
                    request,
                    "dashboard/recruiterDashboard/selectSkills.html",
                    {
                        "form": form,
...
from .models import ApplicantAnswer, Application, CompletedSkills, Job, SkillQuestion
...
def answer_job_questions(request, job_id):
    ...
    if application.has_started:
        messages.warning(
            request,
            "You have already attempted the application for this job. Please contact the admins if you need further assistance.",
        )
    ...
    completed_skills = CompletedSkills.objects.filter(
        user=user, job=job, is_completed=True
    ...
    if request.method == "POST":
        ...
        if answers:
            ...
            existing_answers = set(
                ApplicantAnswer.objects.filter(
                    applicant=user, job=job, application=application, question__in=mcqs
            ...
            with transaction.atomic():
                ...
                CompletedSkills.objects.update_or_create(
                    user=user, job=job, skill=current_skill, defaults={"is_completed": True}
                )
            ...
            messages.success(
                request,
                f"Your answers for the skill '{current_skill.name}' have been recorded. Total score: {total_score}",
            )
            return redirect("job:job_application_success", job_id=job_id)
        else:
        ...
        mcqs = SkillQuestion.objects.filter(
            Q(skill=current_skill)
            & Q(entry_level=job.level)
            & ~Q(option_a="")
            & ~Q(option_b="")
            & ~Q(option_c="")
            & ~Q(option_d="")
            ...
            :5
        ]  # Randomize and limit to 5 questions
        ...
        for question in mcqs:
            logger.debug(
                f"Question ID: {question.id}, Question: {question.question}, Options: {question.option_a}, {question.option_b}, {question.option_c}, {question.option_d}"
            )
...
def save_video(request):
    if (
        ...
        and request.POST.get("application_id")
    ):
...
def get_questions_for_skill(request, skill_id):
    ...
    try:
        ...
        print(
            f"Current application found for job: {job.title}"
        )  # Log successful application retrieval
    except Application.DoesNotExist:
    ...
    if completed_skills:
        print(
            f"User has already completed skill: {skill.name} for job: {job.title}"
        )  # Log completion
        return JsonResponse(
            {
                "questions": [],
                "message": "You have already completed this skill for the current job.",
            }
        )
    ...
    for question in serialized_questions:
        print(
            f"Question ID: {question['id']}, Question: {question['question']}, Options: {question['options']}"
        )
...
def save_responses(request):
    if request.method == "POST":
        try:
            ...
            try:
                current_application = Application.objects.get(
                    user=request.user, job=job, has_completed_quiz=False
                )
            except Application.DoesNotExist:
                return JsonResponse(
                    {"error": "No active job application found for this job"}, status=400
                )
...
def job_application_success(request, job_id):
    ...
    application, created = Application.objects.update_or_create(
        ...
        job=job,
        defaults={
            "quiz_score": request.session.get(
                "quiz_score", 0
            ),  # Assuming quiz_score is stored in session
            "matching_percentage": request.session.get(
                "matching_percentage", 0.0
            ),  # Assuming matching percentage is stored in session
            "overall_match_percentage": request.session.get(
                "overall_match_percentage", 0.0
            ),  # Assuming overall matching percentage is stored in session
        },
    )
    ...
    for app in applications:
        total_questions = SkillQuestion.objects.filter(
            skill__in=job.requirements.all(), entry_level=job.level
        ).count()
    ...
    if application in top_5_applications:
        ...
        send_application_email(
            request.user.email, "Application Received", applicant_message, job_link
        )
    ...
    for app in waiting_list_applications:
        if app != application and app not in applications[:10]:
            ...
            send_application_email(
                app.user.email, "Application Rejected", rejection_message, job_link
            )
...
def job_details(request, job_id):
    ...
    similar_jobs = Job.objects.annotate(
        max_matching_percentage=Avg("application__overall_match_percentage")
    ).filter(max_matching_percentage__gte=5.0)
...
def job_listing_details(request, job_id):
    try:
        ...
        similar_jobs_data = list(
            similar_jobs.values("id", "title", "description")
...
def save_job(request, job_id):
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        ...
        if created:
            return JsonResponse(
                {"status": "success", "message": "Job saved successfully.", "action": "saved"}
            )
        else:
            saved_job.delete()  # Optionally, remove the saved job if it was already saved
            return JsonResponse(
                {"status": "success", "message": "Job removed successfully.", "action": "removed"}
            )
...
def remove_saved_job(request, job_id):
    if request.method == "POST" and request.is_ajax():
        ...
        if saved_job:
            ...
            return JsonResponse(
                {"status": "error", "message": "This job is not in your saved jobs."}
            )
...
def incomplete_jobs_view(request):
    ...
    for job in jobs:
        ...
        if completed_skills.exists() and not all(
            skill in [cs.skill for cs in completed_skills if cs.is_completed] for skill in skills
        ):
