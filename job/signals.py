...
def mark_skill_as_completed(sender, instance, created, **kwargs):
    if created:
        logger.debug(f"Signal triggered for ApplicantAnswer with ID: {instance.id} =================================")
        
        application = instance.application
        skill = instance.question.skill

        logger.debug(f"Checking completion for skill_id: {skill.id} and job_id: {application.job.id}")
