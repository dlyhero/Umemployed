...
urlpatterns = [
    ...
    path(
        "jobs/<int:pk>/", views.JobDetailAPIView.as_view(), name="job_detail"
    ),  # Retrieve job details
    path(
        "jobs/<int:job_id>/apply/", views.ApplyJobAPIView.as_view(), name="apply_job"
    ),  # Apply for a job
    path("jobs/<int:job_id>/save/", views.SaveJobAPIView.as_view(), name="save_job"),  # Save a job
    path(
        "jobs/<int:job_id>/withdraw/",
        views.WithdrawApplicationAPIView.as_view(),
        name="withdraw_application",
    ),  # Withdraw a job application
    path(
        "jobs/<int:job_id>/shortlist/<int:candidate_id>/",
        views.ShortlistCandidateAPIView.as_view(),
        name="shortlist_candidate",
    ),  # Shortlist a candidate
    path(
        "jobs/<int:job_id>/decline/<int:candidate_id>/",
        views.DeclineCandidateAPIView.as_view(),
        name="decline_candidate",
    ),  # Decline a candidate
    path(
        "jobs/<int:job_id>/extracted-skills/",
        views.ExtractedSkillsAPIView.as_view(),
        name="extracted_skills",
    ),  # Get extracted skills for a job
    path(
        "jobs/<int:job_id>/tailored-description/",
        views.TailoredJobDescriptionAPIView.as_view(),
        name="tailored_job_description",
    ),  # Get tailored job description
    # Saved jobs
    ...
    path(
        "generate-questions/", views.GenerateQuestionsAPIView.as_view(), name="generate_questions"
    ),  # Generate questions for skills
    path(
        "extract-technical-skills/",
        views.ExtractTechnicalSkillsAPIView.as_view(),
        name="extract_technical_skills",
    ),  # Extract technical skills from job description
    # Applied jobs
    path(
        "applied-jobs/", views.AppliedJobsListAPIView.as_view(), name="applied_jobs"
    ),  # List applied jobs
    # Job creation steps
    path(
        "create-step1/", views.CreateJobStep1APIView.as_view(), name="create_job_step1"
    ),  # Step 1: Create a job
    path(
        "<int:job_id>/create-step2/", views.CreateJobStep2APIView.as_view(), name="create_job_step2"
    ),  # Step 2: Update job preferences
    path(
        "<int:job_id>/create-step3/", views.CreateJobStep3APIView.as_view(), name="create_job_step3"
    ),  # Step 3: Update job description
    path(
    ...
    ),  # Fetch job-related options
    # Search jobs
    path("jobs/search/", views.SearchJobsAPIView.as_view(), name="search_jobs"),  # Search for jobs
    # Report test issues
    path(
        "<int:job_id>/report-test/", views.ReportTestAPIView.as_view(), name="report_test_api"
    ),  # Report test issues
    # Job questions
    path(
