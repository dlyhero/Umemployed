...
def save_skills_to_database(job_title, skills):
    ...
    try:
        ...
        for skill_name in skills:
            ...
            skill.is_extracted = (
                True  # Set the flag indicating that this skill was extracted from a job description
            )
            skill.save()

    except Exception as e:
        logger.error(
            "An error occurred while saving skills to the database for %s: %s", job_title, e
        )
...
def extract_technical_skills(job_title, job_description):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4", messages=conversation, timeout=120  # Extended timeout to 120 seconds
        )
...
def extract_technical_skills_endpoint(request):
    ...
    if result is None:
        return JsonResponse(
            {"error": "An error occurred while extracting technical skills."}, status=500
        )
