    ...
    if request.method == "GET":
        ...
        try:
            ...
            for skill_name in selected_skill_names:
                questions = await generate_questions_for_skills(
                    job_title, entry_level, skill_name, questions_per_skill
                )
    ...
    try:
        ...
        response = await loop.run_in_executor(
            ...
            lambda: openai_client.chat.completions.create(
                model="gpt-4", messages=conversation, timeout=120
            ),
        )
...
async def query_gemini(conversation):
    ...
    try:
        response = await asyncio.get_event_loop().run_in_executor(
            None, lambda: genai_model.generate_content(conversation[0]["content"])
        )
