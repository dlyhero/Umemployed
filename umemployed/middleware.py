...
class RedirectBasedOnRoleMiddleware:
    ...
    def __call__(self, request):
        ...
        if request.user.is_authenticated:
            ...
            elif request.path == reverse("switch_account_type") and (
                request.user.is_applicant or request.user.is_recruiter
            ):
                return redirect(
                    reverse("home")
                )  # Redirect to home if already applicant or recruiter
