        ...
        if form.is_valid():
            ...
            try:
                ...
                if blob_client.exists():
                    ...
                    messages.info(
                        request, "File already exists in storage. Using the existing file."
                    )
                else:
                ...
                transcript, created = Transcript.objects.get_or_create(
                    user=request.user, file=blob_name, defaults={"extracted_text": ""}
                )
                ...
                if "BlobAlreadyExists" in str(e):
                    return JsonResponse(
                        {"error": "File already exists in storage. Using the existing file."},
                        status=400,
                    )
                else:
                    return JsonResponse(
    ...
    try:
        ...
        if not transcript:
            transcript = Transcript.objects.create(
                user=request.user, file=file_path, extracted_text=""
            )
        ...
        if transcript.extracted_text and transcript.reasoning:
            ...
            extracted_text = (
                extracted_text.replace("\x00", "") if extracted_text else extracted_text
            )
        ...
        return render(
            request,
            "resume/transcript/transcript_result.html",
            {
        ...
        messages.error(
            request, "An error occurred while processing the transcript. Please try again."
        )
