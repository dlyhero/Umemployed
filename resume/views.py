...
from .models import ContactInfo, Resume, ResumeDoc, Skill, SkillCategory

geocoder = OpenCage(
    "70d694d4b6824310a0a7e3a4f5041ce3"
...
def update_resume(request):
    ...
    try:
        ...
        resume = Resume(
            user=request.user, job_title="Default Title"
        )  # Use a default job title or other defaults
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            resume.first_name = (
                contact_info.name.split()[0] if contact_info.name else resume.first_name
            )  # Assuming first_name is the first part of the full name
            resume.surname = (
                " ".join(contact_info.name.split()[1:]) if contact_info.name else resume.surname
            )  # Assuming surname is the rest of the name
            resume.phone = contact_info.phone
            resume.country = (
                contact_info.country.name if contact_info.country else resume.country
            )  # CountryField is different from CharField, use .name for its value
...
def selec_skills(request):
    ...
    if request.method == "POST":
        ...
        if selected_category_id:
            ...
            form = SkillForm(
                request.POST, category=selected_category, user=request.user
            )  # Pass user parameter
            if form.is_valid():
                resume = Resume.objects.get(user=request.user)
        ...
        if request.user.is_applicant:
            ...
            if selected_category_id:
                selected_category = SkillCategory.objects.get(id=selected_category_id)
                form = SkillForm(
                    category=selected_category, user=request.user
                )  # Pass user parameter
...
def get_matching_jobs(user_job_title, user_skills):
    ...
    for job in all_jobs:
        ...
        if job_skills.intersection(user_skills):
            ...
            proximity_percentage = calculate_proximity_percentage(
                user_job_title, job.location
            )  # Fix the parameter to user_job_title
            total_match_percentage = match_percentage + proximity_percentage
            matching_jobs.append(
                {
                    "job": job,
                    "match_percentage": total_match_percentage,
...
def display_matching_jobs(request):
    ...
    if user.has_resume:
        ...
        user_skills = set(
            user_resume.skills.all()
        )  # Assuming skills are stored in a ManyToManyField
        ...
        for job_data in matching_jobs_data:
            ...
            match_percentage, missing_skills = calculate_skill_match(user_skills, job_skills)
            job_dict = {
                "company": {"logo": {"url": job.company.logo.url}, "name": job.company.name},
                "id": job.id,
                "title": job.title,
