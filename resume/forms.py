...
class ContactInfoForm(forms.ModelForm):
    ...
    other_job_title = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Enter job title if not in list"}
        ),
    )
...
class UpdateResumeForm2(forms.ModelForm):
    category = forms.ModelChoiceField(
        queryset=SkillCategory.objects.all().order_by("name"), empty_label=None
    )
    skills = forms.ModelMultipleChoiceField(
        queryset=Skill.objects.none(), widget=forms.CheckboxSelectMultiple
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        unique_skills_qs = Skill.objects.filter(
            Q(id__in=[skill.id for skill in unique_skills])
        ).order_by("name")
